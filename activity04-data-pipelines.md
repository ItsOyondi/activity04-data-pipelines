...
**Response**: After knitting, when ‘Message’ is set to false, the error
“ℹ Use `spec()` for the full column specifications.” is not included in
the output file while when set to true, the message is displayed within
the R code chunk.
...
**Response**: The ‘message’ option is used in the code chunk to hide or
show column specifications information the the data displayed.
...
**Response**: The key variables that could be used in determining the
major wth the lowest unemployment rate include: major, total, rank, and
unemployment rate. Steps: Using excel, the first step is to select the
columns with the main variables. Then, organize the data (rows) based on
the unemployment rate and major in descending order to display the
lowest unemployment rate first displayed in the first row.
